...
export default function FloatingContact() {
    ...
    const phoneLinks = [
        { href: "tel:+201007006081", label: t('phone_edu_psych_special'), number: "01007006081" },
        { href: "tel:+201091010454", label: t('phone_edu'), number: "01091010454" },
        { href: "tel:+201093998000", label: t('phone_psych_special'), number: "01093998000" }
    ];
    ...
    return (
        <div className="fixed bottom-6 right-6 z-[100] flex flex-col gap-4 items-end animate-in fade-in slide-in-from-bottom-10 duration-500">
            ...
            >
                <div className="bg-white dark:bg-[#0F172A] rounded-2xl shadow-2xl border border-[#25D366]/20 overflow-hidden w-[260px]">
                    <div className="p-3 bg-[#25D366]/10 border-b border-[#25D366]/10">
                        <p className="text-[10px] font-black uppercase tracking-widest text-[#128C7E] dark:text-[#25D366] text-center">
                            {t('wa_title')}
                        </p>
                    </div>
                    <div className="flex flex-col">
                        {phoneLinks.map((p, i) => (
                            <a 
                                key={i}
                                href={`https://wa.me/${p.number.replace(/[^0-9]/g, '')}`}
                                target="_blank"
                                rel="noopener noreferrer"
                                className="flex flex-col p-4 hover:bg-[#25D366]/10 transition-colors border-b last:border-0 border-slate-100 dark:border-white/5 group"
                            >
                                <span className="text-[10px] font-bold text-slate-400 group-hover:text-[#128C7E] dark:group-hover:text-[#25D366] transition-colors">{p.label}</span>
                                <div className="flex items-center justify-between">
                                    <span className="text-sm font-black text-[#1e3a8a] dark:text-white" dir="ltr">{p.number}</span>
            ...
            >
                <div className="bg-white dark:bg-[#0F172A] rounded-2xl shadow-2xl border border-[#D4A853]/20 overflow-hidden w-[260px]">
                    <div className="p-3 bg-[#D4A853]/10 border-b border-[#D4A853]/10">
                        <p className="text-[10px] font-black uppercase tracking-widest text-[#D4A853] text-center">
                            {t('phone_title')}
                        </p>
                    </div>
                    <div className="flex flex-col">
                        {phoneLinks.map((p, i) => (
                            <a 
                                key={i}
                                href={p.href}
                                className="flex flex-col p-4 hover:bg-[#D4A853]/10 transition-colors border-b last:border-0 border-slate-100 dark:border-white/5 group"
                            >
                                <span className="text-[10px] font-bold text-slate-400 group-hover:text-[#D4A853] transition-colors">{p.label}</span>
                                <span className="text-sm font-black text-[#1e3a8a] dark:text-white" dir="ltr">{p.number}</span>
                            </a>
                        ))}
                    </div>
