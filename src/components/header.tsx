        ...
        <header className="fixed top-0 left-0 right-0 z-50 transition-all duration-500 pt-2 md:pt-4 px-1.5 md:px-4 isolate">
            <div className="max-w-7xl mx-auto bg-white/80 dark:bg-slate-900/80 backdrop-blur-2xl rounded-[2rem] md:rounded-[2.5rem] shadow-[0_8px_32px_rgba(0,0,0,0.06)] dark:shadow-[0_8px_32px_rgba(0,0,0,0.3)] px-4 sm:px-6 md:px-12 py-2 md:py-3 flex items-center justify-between border border-white/60 dark:border-white/10 ring-1 ring-black/[0.02] dark:ring-white/[0.05] transition-all duration-700 hover:bg-white/90 dark:hover:bg-slate-900/90 hover:shadow-[0_8px_32px_rgba(30,58,138,0.1)]">
                ...
                <Link href="/" className="flex items-center gap-2 sm:gap-4 relative group/logo transition-all duration-500 shrink-0 touch-action-manipulation">
                    <div className="relative flex items-center gap-2 z-10 group-hover/logo:scale-105 transition-transform duration-500 pointer-events-none">
                        <Image
                            src="/About.png"
                            alt="المؤسسة المصريه"
                            width={54}
                ...
                <div className="flex items-center gap-1 sm:gap-4 md:gap-6">
                    <Link
                        href="/#contact"
                        className="bg-gradient-to-r from-[#1e3a8a] to-[#2563eb] text-white px-5 sm:px-8 py-2 md:py-3 rounded-full font-black text-[11px] sm:text-base lg:text-lg shadow-[0_10px_20px_-10px_rgba(30,58,138,0.6)] hover:shadow-[0_15px_30px_-10px_rgba(30,58,138,0.8)] active:scale-95 border border-white/20 relative overflow-hidden group flex items-center justify-center whitespace-nowrap magnetic-btn shrink-0 touch-action-manipulation"
                    >
                        <span className="relative z-10 hidden sm:inline text-white pointer-events-none">{t('btn_register')}</span>
                        <span className="relative z-10 sm:hidden text-white pointer-events-none">{t('btn_register_short')}</span>
                        <div className="absolute inset-0 h-full w-full bg-gradient-to-r from-transparent via-white/20 to-transparent -translate-x-[150%] rtl:translate-x-[150%] group-hover:translate-x-[150%] rtl:group-hover:-translate-x-[150%] transition-transform duration-700 ease-in-out pointer-events-none" />
                    </Link>

                    <div className="flex items-center gap-0.5 sm:gap-2">
                        <Link href="/profile" className="w-10 h-10 flex items-center justify-center rounded-xl hover:bg-slate-100 dark:hover:bg-white/5 transition-all text-slate-600 dark:text-slate-400 hover:text-[#1e3a8a] dark:hover:text-[#D4A853] relative group overflow-hidden" title="الملف الشخصي">
                            {userAvatar ? (
                                userAvatar.startsWith('/') || userAvatar.startsWith('http') ? (
                                    <div className="relative w-8 h-8 rounded-lg overflow-hidden group-hover:scale-110 transition-transform duration-500 shadow-md">
                                        <Image 
                                            src={userAvatar} 
                                            alt="Avatar" 
                                            fill 
                                    ...
                                    <span className="text-xl group-hover:scale-125 transition-transform">{userAvatar}</span>
                                )
                            ) : (
                    ...
                    <button
                        ...
                        className="md:hidden p-2 sm:p-3 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors active:scale-90 touch-action-manipulation"
                        aria-label="فتح القائمة"
                    >
                        <svg className="w-6 h-6 text-slate-700 dark:text-slate-200 pointer-events-none" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            {isMenuOpen ? (
                ...
                <div className="md:hidden mt-3 bg-white/80 dark:bg-slate-900/80 backdrop-blur-2xl rounded-[2rem] shadow-[0_20px_50px_rgba(0,0,0,0.1)] dark:shadow-[0_20px_50px_rgba(0,0,0,0.4)] p-4 border border-white/60 dark:border-white/10 animate-in fade-in zoom-in-95 duration-300">
                    <nav className="flex flex-col gap-2">
                        ...
                        <Link
                            ...
                            className="text-slate-700 dark:text-slate-200 hover:text-[#1e3a8a] dark:hover:text-[#D4A853] transition-all font-bold py-5 px-6 rounded-2xl hover:bg-white/50 dark:hover:bg-white/5 flex items-center justify-between group active:bg-[#D4A853]/10 touch-action-manipulation"
                        >
                            <span className="pointer-events-none">{t('nav_about')}</span>
                            <span className="opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">→</span>
                        </Link>
                        <Link
                            href="/#contact"
                            ...
                            className="text-slate-700 dark:text-slate-200 hover:text-[#1e3a8a] dark:hover:text-[#D4A853] transition-all font-bold py-5 px-6 rounded-2xl hover:bg-white/50 dark:hover:bg-white/5 flex items-center justify-between group active:bg-[#D4A853]/10 touch-action-manipulation"
                        >
                            <span className="pointer-events-none">{t('nav_contact')}</span>
                            <span className="opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">→</span>
                        </Link>
                        <Link
                            href="/forum"
